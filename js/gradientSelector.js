/*
 * JS for my gradient selector, another project on GitHub
 */
var gradients = [];
(function(){
    ...
    function GradientInput(element){
        ...
        var offset=10;
        this.offset=10;
        if(!(this.width=element.getAttribute("width"))){
            if(!element.style.width){
                element.style.width="276px";
            }
            this.width=element.style.width.substr(0, element.style.width.length -2);
        }
        this.width =parseInt(this.width)-offset*6;
        if(!(this.height=element.getAttribute("height"))){
            if(!element.style.height){
                element.style.height="64px";
            }
            this.height=element.style.height.substr(0, element.style.width.length -2);
        }
        this.height=parseInt(this.height)-offset*2;
        ...
        this.setPallet = function(markerData){
            ...
            for(var i=0;i<markerData.length;i++){
                this.markers[i]= new GradientMarker(this.ctx, this.width, this.height,
                                                    ...
                                                    {position:(i==0)? 0 : ((i==markerData.length-1)? 1 : markerData[i].position),
                                                    locked:(i==0 || i==markerData.length-1),
        ...
        this.randomPallet = function(){
            this.setPallet([{color:randomColor()},{color:randomColor()}]);
        }
        this.printPallet = function(){
            var data=[];
            for(var i=0;i<this.markers.length;i++){
                data[i]={color:this.markers[i].color, position:this.markers[i].position};
            }
            
            console.log(JSON.stringify(data));
        }
        this.getPalletData = function(){
            var data=[];
            for(var i=0;i<this.markers.length;i++){
                data[i]={color:this.markers[i].color, position:this.markers[i].position};
            }
